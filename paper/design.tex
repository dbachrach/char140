...
\hlfixme{Mention that a salt is not really useful here since the
  attacker needs to do decryptions, not comparisons, and thus cannot do
  pre-computation for a rainbow table. He could pre-compute and store
  all the keys, but the benefit is at most a savings of half. Note that
  it is useful if we make hashing slow -- goes in discussion.}
